...
EthernetMessageParser::EthernetMessageParser(const Napi::CallbackInfo &info)
    : ObjectWrap(info)
{
...
Napi::Value EthernetMessageParser::Receive(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
...
void EthernetMessageParser::Init(Napi::Env env, Napi::Object exports)
{
...
int EthernetMessageParser::_parse_nmea(uint8_t data) {
    int ret=0;
...
int EthernetMessageParser::_parse_user_packet_payload(uint8_t* buff, uint32_t nbyte){
    int ret = 1;

    uint8_t payload_length = buff[2]; //TODO
    char packetType[4];
...
int EthernetMessageParser::_accept(uint8_t data){
    int ret = 0;
    if (_userRaw.flag == 0) {
